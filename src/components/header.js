...
const Head = styled.header`
  ...
  align-items: center;
  background-color: ${props => props.bgColor ? props.bgColor : theme.colors.clearlinkBlue };
  padding-left: 1rem;
  padding-right: 1rem;
...
const Header = ({ data, bgColor }) => {
  ...
  const three = () => <HeaderCopy style={{ transitionDelay: '300ms' }}>{data.copy}<HeaderLink href="https://www.naganithin.netlify.app" target="_blank">{data.company}</HeaderLink></HeaderCopy>;
  ...
  return (
    <Head bgColor={bgColor}>
      ...
      <Container>
        <Image filename="myimage.jpeg" classes='gatsby-sbs-image' alt="Send me a message and let's connect." />
      </Container>
    </Head>
  )
